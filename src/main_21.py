    ...
    with torch.no_grad():
        for time_idx, test_snap in enumerate(tqdm(test_list,colour='RED')):
            ...
            if args.add_static_graph:
                all_tail_seq = sp.load_npz('./data/{}/{}/h_r_history_seq_{}.npz'.format(args.dataset, test_data, str(query_time)))
...
#        optim.Adam(cond_pred_model.parameters(), lr=0.001)
        for epoch in range(args.n_epochs):
            ...
            for batch_idx, train_sample_num in enumerate(tqdm(idx)):
                ...
                sequence = np.load('./data/{}/train_seq/h_r_seen_triple_{}.npy'.format(args.dataset,str(train_times[train_sample_num]))
                ...
                seq_idx = true_triples[:, 0] * num_rels * 2 + true_triples[:, 1] 
                if args.add_static_graph:
                    all_tail_seq = sp.load_npz('./data/{}/train_seq/h_r_history_seq_{}.npz'.format(args.dataset, str(train_times[train_sample_num-1])))
                    history_tail_seq = torch.Tensor(all_tail_seq[seq_idx.cpu()].todense())
                    one_hot_tail_seq = history_tail_seq.masked_fill(history_tail_seq != 0, 1)
                    if use_cuda:
                        history_tail_seq, one_hot_tail_seq = history_tail_seq.to(args.gpu), one_hot_tail_seq.to(args.gpu)
                ...
                loss = loss_diffu_value / accumulation_steps  
                losses.append(loss.item())
                
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_norm)  # clip gradients
                if (batch_idx + 1) % accumulation_steps == 0 or batch_idx == len(idx)-1:
...
#                            "hit_filter":test_filter_hit_obj,"epoch":epoch}})
            log_message = "Epoch {:04d}, AveLoss: {:.4f}, BestValMRR {:.4f}, BestTestMRR: {:.4f}, test_hit@1: {:.4f}, test_hit@3: {:.4f}, test_hit@10: {:.4f}, Model: {}, Dataset: {}".format(
    epoch, np.mean(losses), best_val_mrr, best_test_mrr, best_test_hit[0], best_test_hit[1], best_test_hit[2], model_name, args.dataset)
            print(log_message)
