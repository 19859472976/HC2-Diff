...
def compare_distributions(data1, data2, filename='distribution_comparison.png'):
    ...
    bins = np.linspace(min(min(data1_flat), min(data2_flat)), max(max(data1_flat), max(data2_flat)), 30)


    plt.subplot(1, 2, 1)
    

    sns.histplot(data1_flat, kde=True, color="skyblue", stat="density", bins=bins, label='Gauss')
    sns.histplot(data2_flat, kde=True, color="orange", stat="density", bins=bins, label='Cond Gauss')
    ...
    # Q-Q
    plt.subplot(1, 2, 2)
    stats.probplot(data1_flat, dist="norm", plot=plt)
    stats.probplot(data2_flat, dist="norm", plot=plt)
...
def draw(noise ,name):
	...
	noise_np = noise.squeeze(1).detach().cpu().numpy()
#	noise_np = noise_2d.numpy()

	plt.rcParams.update({'font.size': 20})
	plt.figure(figsize=(10, 6), dpi=300)
	heatmap = plt.imshow(noise_np, cmap="winter", aspect="auto",interpolation="nearest")
	plt.colorbar(heatmap, label="Noise Intensity")
#	plt.xlabel("Width (200 dimensions)")
#	plt.ylabel("Height (324 dimensions)")
#	plt.title("Noise Heatmap")
...
class DiffuRec(nn.Module):
    ...
    def q_sample(self, x_start, t, cond_pred_model, embeddings_dict, noise=None, mask=None):
        ...
        cond_noise,attn_output = cond_pred_model(noise, embeddings_dict['object'] + embeddings_dict['relation'])
#        print(cond_noise)
#        draw(attn_output,"noise_cond.png")
#        draw(cond_noise,"noise_gauss.png")
#        compare_distributions(noise.detach().cpu().numpy(), cond_noise.detach().cpu().numpy())
#        exit()
#        cond_pred_model(noise, embeddings_dict['object'] + embeddings_dict['relation'])
        assert noise.shape == x_start.shape
        x_t = (
            _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start
#            + (1 - _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape)) * x_start
            + _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
            * attn_output  ## reparameter trick
    ...
    def q_sample_adj(self, x_start, t, noise=None, mask=None, adj_matrix=None):
        ...
        if adj_matrix is not None:
            x_t = torch.matmul(adj_matrix, x_t)
