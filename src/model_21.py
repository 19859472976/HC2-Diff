...
class CurriculumScheduleSampler:
    ...
    def get_stage_bounds(self):
        """动态划分噪声阶段"""
        if self.current_step < self.total_steps * 0.3:
...
class Att_Diffuse_model(nn.Module):
    ...
    def diffu_rep_pre(self, rep_diffu,e_embs,history_tail_seq=None, one_hot_tail_seq=None):
        ...
        if history_tail_seq != None and self.seen_addition:
            ...
            temp_mask = torch.tensor(np.array(one_hot_tail_seq.cpu() == 0, dtype=float) * (-1e9)).to(self.gpu)
    ...
    def forward(self, glist, sequence, tag, train_f=True,use_cuda=True,y_0_hat_batch = None,seen_before_label=None,static_graph=None): 
        ...
        if self.use_static:
            ...
            static_graph.ndata['h'] = torch.cat((self.emb_ent, self.words_emb), dim=0)
        ...
        object_sequence = sequence[:,-self.max_len:,0]
        relation_sequence = sequence[:,-self.max_len:,1]
        max_time = torch.max(sequence[:,-self.max_len:,2],dim=1,keepdim=True)[0]
        time_sequence = max_time - sequence[:,-self.max_len:,2]
...
#        time_embeddings_re = self._time2vec(time_diff)
        time_embeddings_re = self.time_embeddings.weight[time_sequence]
        mask_seq = (time_sequence<=max_time+2).float()
...
#        model_optim.zero_grad()
        sequence = utils.select_relevant_slices_v1(tag, sequence, entity_dict, rel_dict, 64)
#        print(tag)
#        print(sequence)
#        exit()
#        sequence = sequence[:,-64:,:]
#        exit()
        object_sequence = sequence[:, :, 0]
        relation_sequence = sequence[:, :, 1]
        max_time = torch.max(sequence[:, :, 2], dim=1, keepdim=True)[0]
        time_sequence = max_time - sequence[:, :, 2]
#        print(time_sequence)
#        print(max_time)
...
#            inter_embeddings = gate * object_embeddings + (1 - gate) * (time_embeddings_re + relation_embeddings)
		    ...
		    'object': object_embeddings.clone(),
		    'relation': relation_embeddings.clone(),
		    'time': relation_embeddings.clone()
		   }
            inter_embeddings = self.gate(object_embeddings, time_embeddings_re, relation_embeddings)
