...
class FeatureFusionTransformer(nn.Module):
    def __init__(self, d_model=200, nhead=6, num_layers=2):
        super().__init__()
...
class CrossAttentionFusion(nn.Module):
    def __init__(self, noise_dim, history_dim, num_heads=10, dropout=0.1):
        super().__init__()
    ...
    def forward(self, base_noise, c_history):
        ...
        # self.norm(
        cond_noise = base_noise + self.res_weight * attn_output #)
        return cond_noise,attn_output
...
#        print(hidden_state.shape)
        hidden_candidate = self.hidden_trans(torch.cat([factor, reset_value * hidden_state], dim=-1))
        hidden_state = (1 - update_value) * message + update_value * hidden_candidate
        return hidden_state
